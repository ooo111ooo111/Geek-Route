...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void refreshCache() {
        System.out.println("缓存大小：" + categoryCaches.size());

        List<CategoryDO> list = categoryDao.listAllCategoriesFromDb();
        System.out.println("从数据库加载分类数量：" + list.size());

        categoryCaches.invalidateAll();
        categoryCaches.cleanUp();
        list.forEach(s -> {
            CategoryDTO dto = ArticleConverter.toDto(s);
            System.out.println("转换分类：" + s.getCategoryName() + " -> " + dto);
            categoryCaches.put(s.getId(), dto);
        });
    }
